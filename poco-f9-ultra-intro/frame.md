...
description: >
  ...
  identical and sacred — the near-black ground with white ink, POCOTech 400, the
  universal 1px POCO-yellow hairline as the only structural device, compass-drafted geometric rings,
...
typography:
  # — reading ramp (MiSans Latin) —
  body:        { fontFamily: "POCOTech", cqw: 1.0,  weight: 400, lineHeight: 1.6, color: "text-secondary" }
  body-sm:     { fontFamily: "POCOTech", cqw: 0.85, weight: 400, lineHeight: 1.6 }
  subtitle:    { fontFamily: "POCOTech", cqw: 1.3,  weight: 400, lineHeight: 1.5 }
  label:       { fontFamily: "POCOTech", px: 14, weight: 500, tracking: "3px", upper: true, color: "accent" }
  attribution: { fontFamily: "POCOTech", px: 15, weight: 400, tracking: "2px", upper: true, color: "accent" }
  micro:       { fontFamily: "POCOTech", px: 12, weight: 400, tracking: "2px", upper: true, color: "accent" }
  # — display / hero ramp (MiSans Latin 400, sentence case) —
  h3:          { fontFamily: "POCOTech", cqw: 1.8, weight: 400, lineHeight: 1.1 }
  timeline-headline:{ fontFamily: "POCOTech", cqw: 1.9, weight: 400, lineHeight: 1.1 }
  card-headline:{ fontFamily: "POCOTech", cqw: 2.0, weight: 400, lineHeight: 1.15 }
  stat-figure: { fontFamily: "POCOTech", cqw: 3.0, weight: 400, lineHeight: 1.0 }
  quote-mark:  { fontFamily: "POCOTech", cqw: 9.0, weight: 400, lineHeight: 0.5, color: "line" }
  h2:          { fontFamily: "POCOTech", cqw: 4.0, weight: 400, lineHeight: 1.1 }
  h1:          { fontFamily: "POCOTech", cqw: 6.2, weight: 400, lineHeight: 1.06 }
  display:     { fontFamily: "POCOTech", cqw: 8.0, weight: 400, lineHeight: 1.04 }
...
- **Fonts** — display AND body are **POCOTech** (the brand's staged display face, local woff in `assets/fonts/`; the captured MiSans Latin files are hash-named variable-font subsets that cannot be wired face-by-face). Ignore any preset font name (Playfair, Inter, MiSans) lingering in prose.
- **Weights** — POCOTech stages `{300, 400, 500, 600}` locally; every weight is clamped to these — the 400-weight display doctrine holds.
- **Colors** — the brand is dual-register (dark product sections + white spec sheets, near-tied in area). This video lives on the DARK register — dark footage, white title — so the palette is the brand's dark half: near-black ground, white ink, POCO-yellow accent and hairlines. Preset color NAMES in prose ("warm stone", "taupe") mean the remapped dark-register values.
...
The voice is a literary pairing: **MiSans Latin** at weight 400 (the thin-stroke didone, never
bold, always sentence case) carries every headline, numeral, and quote mark in ink; **MiSans Latin**
...
- **MiSans Latin 400** (ink, sentence case) for display; **MiSans Latin** body (gray) + labels (taupe, tracked).
...
Two ramps. The **reading ramp** (MiSans Latin body 1.0cqw gray, labels in px taupe) carries copy + chrome;
...
- **Playfair at 400, ink, sentence case** — never bold, never uppercase, never taupe. **MiSans Latin labels uppercase, 2–3px tracked, taupe.** Italic via Playfair italic for emphasis only.

## Depth & Surface

The flat plane is the only technique. Hierarchy from:

- **Type contrast** — Playfair serif vs MiSans Latin sans; the 8cqw→0.7cqw scale.
...
**Ground** `{colors.bg-primary}`, `pad-x`. **Composes** geo-ring (right, ~34cqw, solid+dashed), vertical-line (left), label, display/h1. **Focal** a 2–3 line Playfair `display`/`h1` headline in ink (italic on the key word), left-anchored, with a taupe `label` above and an MiSans Latin subtitle below. **Chrome** optional bottom meta row (Playfair value + taupe label). **Accent** the geo ring; optionally one `horizontal-accent` ink line (if it clears the meta). **Silence** ~55% empty. **Fixed** Playfair 400 ink sentence-case, ≤2 geo elements, 1px lines. **Free** title, ring placement, meta. **Density** sparse.
...
- **POCOTech (local woff) only.** CJK pairing (Noto Serif SC 700/400) carries over; POCOTech has no italic — substitute weight/color for emphasis.
