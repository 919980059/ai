...
**① 小米澎程 SkyNomad N90**："帮我给 https://www.xiaomiev.com/skynomad/n90 也创作一个高端的宣传视频，主题是 SkyNomad。" —— 交付 **32.1s · 中文七幕 ·「一车一世界」**，纯音乐混音 −15.1 LUFS（综合响度）。
...
*SkyNomad N90 32s 叙事片 ｜ POCO F9 Ultra 10s 介绍片*
...
| 界面 | 出场 | 质感 |
|-|-|-|
| 📜 **纸界** | F2–F5 | 纸白编辑版面：单一墨色、1px 发丝线、卡片大圆角 + 暖黑浮起影（面圆线直） |
...
| 收获 | 实例锚点 |
|-|-|
| 📝 **视频第一次成为文本工程** | 每支片子是 workspace 下的一个目录：HTML 可 diff、可 review、可版本化。v6 → v7 的混音修改在 git 中只是几行属性值 diff；F5 拉伸修复则是删掉两段 rotateY 补间、去掉一条 perspective ——「背景音有点吵」「图片会突然拉伸一下」这类主观反馈，第一次有了精确的对应物。 |
| 🤝 **AI 协作的方式改变了** | 需求各只有一行，全流程（读产品页、分镜、画面、配音、混音、验收）由 Agent 完成，人只承担两个动作 —— 提出需求、给出反馈；skill 体系将「做视频」拆解为 Agent 可执行的操作手册，而非要求人学一个新软件。 |
| ♻️ **流程可复制，启动成本趋近于零** | 换产品（手机 → SUV）、换语言（英文 → 中文）、时长放大三倍，主链原样可跑；验收清单与数字帧语言被直接引用 —— 方法论不需要为新产品重学任何东西。 |
| ✂️ **做减法也是交付** | SkyNomad 旁白在「官方语音库不可用」处走到尽头，纯音乐版反而观感最好；末帧原图直出同理。约束出现时，「砍掉哪个维度」与「做好剩下的维度」是同级工程决策 —— 交付的是约束之下最好的片子。 |
| 📐 **验收可以精确到帧与 dB** | 末三帧亮度、频段对比度、ASR 全对 —— 不是修辞，而是可复现的测量；这正是确定性的价值：同样的话无法对一条时间线说。 |
