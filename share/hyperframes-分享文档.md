...
**② POCO F9 Ultra**（10s 英文介绍片）："为线上产品 https://www.mi.com/global/product/poco-f9-ultra/ 创建一个 10 秒的产品介绍视频，包含淡入标题、背景视频和舒缓的背景音乐。" —— 交付 10.74s / 1920×1080 / 323 帧 / 8.7MB，18 词配音 ASR 回听全对。

**全程未使用任何剪辑软件。**
</callout>

[成片一 · SkyNomad N90（v2）](../skynomad-n90-promo/renders/video-v2.mp4) ｜ [成片二 · POCO F9 Ultra（v13）](../poco-f9-ultra-intro/renders/poco-f9-ultra-intro_2026-09-10_13-08-43.mp4)

*两支成片 —— 需求各一行，全流程 Agent 完成，人只负责提需求与给反馈*

官方资料已足够完备：[GitHub 仓库](https://github.com/heygen-com/hyperframes) 完整文档、[30 天教程](https://hyperframes.heygen.com/thirty-days) 从入门到进阶。本文不复述，只聚焦两点：**两支片子的完整制作过程**，与**实践之后对这个工具的评估**。
...
HyperFrames 的制作是一条主链：**读产品页 → 分镜定节拍 → 画面 → 配音对帧 → 混音 → 机器验收**。实例取自顶部的两支成片 —— POCO F9 Ultra（10s 英文介绍片）与 SkyNomad N90（30s 中文叙事片），后者需求里的那个「也」字就来自前者的交付。
...
一个反直觉的事实：**换一个声音，动的是整条时间线** —— SkyNomad v3 换音色后语速快约 20%，七帧的全部动画 cue、帧长与总时长随之重排（37.0s → 30.5s），而这在源文件里只是一轮可 diff 的修改。
...
| v3 | 换 `zf_xiaoyi`（轻盈柔美）+ 舒缓 BGM（候选中起音密度最低的一支，2.70/s）+ 去字幕 | "人音还是太差了，可以使用官方语音库吗" |
| v4 | 切换 HeyGen 官方语音库 | 服务不可用 |
| **终版** | **砍掉全部人声与音效，音乐独奏**（−15.1 LUFS / 峰值 −1.7 dBTP）+ 末帧原图直出 | "这一版可以了" |
...
最终选择做减法：砍掉全部人声，纯音乐独奏。有意思的是：七幕节拍原本按 VO 词点排布，去掉人声后**画面零改动**，节拍网格依然成立 —— 这一版反而观感最好。

<callout emoji="bulb" background-color="light-yellow" border-color="yellow">
🧠 **做减法也是交付**。官方语音库不可用是硬约束，继续打磨本地 TTS 是死路 —— 于是整支片子改为纯音乐叙事，「科技潮流 × 高端舒适」全部交给音乐承担。末帧撤掉全部调色、直接放原始车图是同一个逻辑：观众看到正确的车型，比看到统一的电影感更重要。
</callout>

### 🎯 4. 精修与验收：把观感变成数字

✨ 精修是同构的另一例。POCO v12 的反馈只有一句："需要看着更高端一点。" 同样**先测量**：逐个验证调色槽位后发现，bloom（光晕）是阈值门控的（约 170 luma），这条氛围素材全场最亮 90 —— 强度拉满也一像素不变，结构性死路。修复不是硬调参数，而是**把光晕烘进资产生成器**（亮核离线高斯晕散，按金色 tint 的通道导数加权加回）：
...
> 🎨 两张流程图均为单文件 HTML+CSS 绘制（源文件与本文同目录 `flow-*.html`），经 Chrome 无头截图输出 4000px 宽 PNG：`chrome --headless=new --force-device-scale-factor=2 --window-size=2000,<高> --screenshot=out.png flow-xxx.html` —— 图与成片一样，都由修改源文件直接产生，这本身也是"文本工程"的一个注脚。
...
这条主链的每一环都可被裁剪，主链本身不可以 —— SkyNomad 的声音环就是一例：配音 + 混音流水线循环三轮后从「人声整体退场、音乐独奏」的出口离开，画面环与验收环原样跑完。
...
**5. 📐 验收可以精确到帧与 dB。** 末三帧亮度、频段对比度、ASR 全对、56px、1.2195、md5 —— 不是修辞，而是可复现的测量。这正是确定性的价值：同样的话无法对一条时间线说。
...
| 🚗 本文实战项目 ①（SkyNomad N90，30s） | `~/workspace/AI/skynomad-n90-promo` |
| 📱 本文实战项目 ②（POCO F9 Ultra，10s） | `~/workspace/AI/poco-f9-ultra-intro` |
| 🎨 流程图源文件（HTML+CSS 可复现） | 本目录 `flow-*.html` |

建议的实践路径：先完成官方 30 天教程，随后**不要停留在示例项目** —— 直接选择一个自己的产品 / 文章 / PR，完整复刻这套流程：**读素材 → 定节拍 → 写画面 → 配音对帧 → 测量混音 → 机器验收**。
