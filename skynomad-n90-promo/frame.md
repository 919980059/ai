...
description: >
  ...
  as bloom / panel / tile underprint, MiSans VF display + MiSans VF sans + JetBrains Mono data,
...
typography:
  # — reading + data ramp —
  body:    { fontFamily: "MiSans VF", cqw: 0.85, weight: 340, lineHeight: 1.5, color: "ink" }
  body-lede:{ fontFamily: "MiSans VF", cqw: 0.95, weight: 340, lineHeight: 1.55 }
  micro-label:{ fontFamily: "MiSans VF", px: 13, weight: 340, tracking: "0.18em", upper: true }
  rail-label:{ fontFamily: "MiSans VF", px: 13, weight: 340, tracking: "0.32em", upper: true }
  mono-data:{ fontFamily: "JetBrains Mono", cqw: 0.73, weight: 340, tracking: "0.04em" }
  pagenum: { fontFamily: "JetBrains Mono", px: 13, weight: 340, tracking: "0.08em", opacity: 0.75 }
  # — display ramp (MiSans VF 400, tight, negative tracking) —
  ledger-title:{ fontFamily: "MiSans VF", cqw: 1.55, weight: 340, lineHeight: 1.15 }
  strand-title:{ fontFamily: "MiSans VF", cqw: 1.7, weight: 340, lineHeight: 1.1 }
  headline-sm:{ fontFamily: "MiSans VF", cqw: 2.9, weight: 340, lineHeight: 1.0 }
  headline:{ fontFamily: "MiSans VF", cqw: 4.6, weight: 340, lineHeight: 1.06, tracking: "-0.005em" }
  date-rail:{ fontFamily: "MiSans VF", cqw: 5.0, weight: 340, lineHeight: 0.96, tracking: "-0.005em" }
  display-it:{ fontFamily: "MiSans VF", cqw: 7.0, weight: 340, italic: true, lineHeight: 1.04, tracking: "-0.005em" }
  numeral-md:{ fontFamily: "MiSans VF", cqw: 7.5, weight: 340, lineHeight: 0.92, tracking: "-0.01em" }
  display:{ fontFamily: "MiSans VF", cqw: 14.6, weight: 340, lineHeight: 0.86, tracking: "-0.018em" }
  numeral-jumbo:{ fontFamily: "MiSans VF", cqw: 28.0, weight: 340, lineHeight: 0.84, tracking: "-0.04em" }
...
- **Fonts** — already set to **MiSans VF** (display) / **MiSans VF** (body); ignore any preset font name lingering in prose.
- **Weights** — the brand font ships `{300, 340}` only; every weight is clamped to these — ignore higher preset weights (e.g. 600/700) in prose.
...
The voice is three faces in rigid roles: **MiSans VF** (weight 400, tight line-height,
negative tracking) carries every display, numeral, and quote from 40px to 720px+; **MiSans VF** carries
...
- **MiSans VF 400** display (tight, negative-tracked); **MiSans VF** body + micro-labels; **JetBrains Mono** data.
...
- **Squint** — one MiSans VF moment dominates at 3–6× its neighbor; the sun bloom centers the eye.
...
Two ramps. The **reading/data ramp** (MiSans VF body 0.85cqw ink, micro-labels in px, JetBrains Mono
data) carries copy + chrome; the **display ramp** (MiSans VF `ledger-title` 1.55cqw →
...
- **MiSans VF is weight 400 only** (its contrast is the weight signal), tight line-height (0.84–1.06), negative tracking; **micro-labels uppercase MiSans VF 600, ≥0.16em**; **mono for all numerals/dates**. No bold serif, no second text color, no mono in body/display.
...
headline-sm. **Focal** a single huge MiSans VF `numeral-jumbo` (≈28cqw) dominating, with a
...
**Focal** a 2-line MiSans VF **italic** `display-it` quote in ink, centered, under an
...
yellow-panel, micro-label, headline. **Focal** an MiSans VF `headline` in ink sitting **on
...
- Set every line in **ink**; use **MiSans VF 400** (tight, negative-tracked) for display, **MiSans VF** body, **JetBrains Mono** for all numerals/dates.
- Make every separator a **1px ink hairline** (soft variant for dense rows); flood a **yellow panel** for poster moments (ink on top).
- Keep micro-labels **uppercase MiSans VF 600, 0.16–0.32em**; pin the **pagenum** bottom-right.
...
- No second text color; no bold MiSans VF; no inverted ink grounds.
...
- **Type** — MiSans VF 400 tight negative-tracked, fit-to-measure; micro-labels uppercase 0.16em+; mono numerals; ≥1.4cqw floor.
...
- **MiSans VF + MiSans VF + JetBrains Mono via Google Fonts.** CJK: Smiley Sans (display) / Noto Serif SC (body) / Noto Sans SC (labels); italic serif and mono tabular figures have no exact Hanzi equal — keep ledger dates Latin.
...
## Font loading (staged — full-coverage faces)

The brand fonts ship as local files in `assets/fonts/` — do NOT link Google Fonts. **The
capture's own `cs.*.ttf` files are 200-glyph unicode-range chunks and must never be used as
frame fonts** — the staged `MiSansVF.ttf` is the full 29,414-glyph variable face (wght
100–900, CJK + Latin + punctuation). `JetBrainsMono-Regular.ttf` covers the mono-data /
pagenum roles. Paste this `<style>` into every frame's `<head>`/`<template>` (captions use
the same files) so `font-family` resolves in preview, snapshot, and render alike:
