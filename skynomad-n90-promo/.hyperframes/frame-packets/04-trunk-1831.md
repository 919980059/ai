...
- voiceover: "1831 升后备厢，装下说走就走。"
- duration: 5.5s
- vo_cues: 04.wav @0.20 (4.971s) — 实测：「1831（一千八百三十一）」0.20–1.15 ·「升后备厢」1.15–1.68 · 逗号 1.68–1.95 ·「装下说走就走」1.95–5.07 · VO 尾 5.07
...
- **Scene 2 (0.20–1.68s) 计数** — 「1831」numeral-jumbo 随读音 0→1831 计数上行（counting-dynamic-scale — scale 随值同 ease 增长，读音毕数字毕 ~1.15）；mk-progress-stat 细填充轨道在其下同程填充（stat-bars-and-fills，轨道即「装载线」）；「升后备厢」单位标签 ~1.20 落定。
- **Scene 3 (1.95–3.2s) 兑现** — VO「装下说走就走」→ 后备厢 hero（20.jpg）1.95 短距 direct-into-slot 入右面板；骑行生活方式卡（20-2.jpg）~2.9 续接落位（两步交错级联，均进槽即停）。
- **Scene 4 (3.2–5.5s) 静持** — 数字保持原位（不呼吸），双卡静止；held read 至帧末。
