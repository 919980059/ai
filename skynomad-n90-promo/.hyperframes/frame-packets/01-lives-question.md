# Frame packet: 01-lives-question
...
- voiceover: "一辆车，能装下多少种生活？"
- duration: 4s
- vo_cues: 01.wav @0.30 (3.349s) — 实测整句连续 WAV 0–3.25（句中微顿 2.02–2.07 不分组）；帧内 11 字匀速 ~0.30s/字：「一辆车」0.30–1.19 ·「能装下」1.19–2.08 ·「多少种生活」2.08–3.55 · VO 尾 3.55
- transition_in: cut
- status: outline
...
- **Scene 1 (0.0–0.35s) 冷开定场** — 近黑场；整车 hero（assets/1.jpg，影界低亮度级）全幅经 soft-blur-in 入场（blur→sharp）；**一根连续慢推**自 t=0 起于根节点、贯穿全帧不中断（单相机多相位推进的 push 相位，中途不得 re-push）；小米澎程字标 ghost 置左上 rail（低透明度，rail-label 字级）。此外画面为空。
- **Scene 2 (0.30–1.19s) 第一拍** — VO「一辆车」落点 → 短语以**一个整体**经弹收式入场落于画面中心（smooth 长尾收，无 overshoot），白色 display 字级压在暗色照片上。
- **Scene 3 (1.19–2.6s) 第二拍** — VO「能装下多少种生活？」→ 中心同位换拍：「一辆车」缩小淡出让位，完整问句到达同一中心（kinetic beat 的 token 换拍，阈值切替；换拍本身即节拍）。
- **Scene 4 (2.6–4.0s) 悬问静持** — 问句 held read 静止；相机同一速率继续慢推（subject 在动，非假活性）；至多 subtle jitter 于问句行。帧末：hero 约 1.06× 起始 scale，问句居中，全画面静止。
